...
<?php if ($_SESSION['wpsqt'][$quizName]['details']['finish_display'] == 'Finish message' || $_SESSION['wpsqt'][$quizName]['details']['finish_display'] == 'Both'  ) { ?>
	<?php if (isset($_SESSION['wpsqt'][$quizName]['details']['pass_finish']) &&
		...
		if(substr($quizName, 0,6)=="TRYOUT"){
			$strmsg="Tryout Selesai";
			//$all = $_SESSION['wpsqt'][$quizName]['sections'][$sectionKey]['limit'];
			$reviewmsg = 'Anda menjawab '. $correctAnswers .' soal benar dari '. $totalPoints. ' soal. Terimakasih atas partisipasinya';
		}else{
			$strmsg="Terimakasih sudah menyelesaikan Tes Online";
			$reviewmsg = "";
		}
		?>
		<p class="finish_exam_text"><?php echo $strmsg;?></p>
		<p class="review_exam_text"><?php echo $reviewmsg; ?></p>
		<?php if($reviewmsg<>""): ?>
		<!-- AddThis Button BEGIN -->
		<!--<div class="addthis_toolbox addthis_default_style addthis_32x32_style">-->
                <div class="addthis_toolbox addthis_default_style addthis_32x32_style" addthis:url="http://www.speedyavengers.com" addthis:title="Hai, saya sudah selesaikan tryout ini, bagaimana dengan kamu?">
