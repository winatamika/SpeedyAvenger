<div class="wrap">
	...
	<?php if (isset($_GET['status'])) {
		...
		<div class="tablenav">
			From : <input name="range_from" id="range_from" type="text" value="<?php echo date("Y-m-d", $_range['range_from']); ?>">
			Until : <input name="range_until" id="range_until" type="text" value="<?php echo date("Y-m-d", $_range['range_until']); ?>">

<?php
$args = array( 'taxonomy' => 'widatel', 'hide_empty'=>0 );
$witels = get_terms('widatel', $args);
?>

			<!--Kota : <input name="kota" id="kota" type="text" value="">-->
			<input name="range_submit" id="range_submit" type="submit" value="Filter">
			<script>
			jQuery(function() {
				...
				jQuery( "#range_until" ).datepicker({
					...
					numberOfMonths: 1,
					onClose: function( selectedDate ) {
						jQuery( "#range_from" ).datepicker( "option", "maxDate", selectedDate );
					}
				});
			});
			</script>
		...
		<div>
			Total Peserta : <?php echo $counts['unviewed_count'] + $counts['accepted_count'] + $counts['rejected_count']; ?> <br />
			Total belum terverifikasi : <?php echo $counts['unviewed_count']; ?> <br />
			Total Lulus : <?php echo $counts['accepted_count']; ?> <br />
			Total Tidak Lulus : <?php echo $counts['rejected_count']; ?> <br />
		</div>
		...
		<table class="widefat post fixed" cellspacing="0">
			<thead>
				<tr>
					...
					<th class="manage-column column-title" scope="col"><a href="<?php echo WPSQT_URL_MAIN; ?>&section=results&subsection=quiz&id=<?php echo urlencode($_GET['id']); ?>&orderby=person_name&order=<?php echo $order; ?><?php if (isset($_GET['status'])) { echo '&status='.$_GET['status']; }  if (isset($_GET['username'])) { echo '&username='.$_GET['username']; } ?>">Nama Peserta</a></th>
					<th scope="col" width="85">WITEL</th>
			...
			<tbody>
				...
				<?php 
					$username = esc_html(wp_kses_stripslashes($result['person_name'])); 
...
//print_r($witeldetail);
				...
				<tr>
					...
					<td class="column-title">
						<strong>
							<a class="row-title" href="<?php echo WPSQT_URL_MAIN; ?>&section=results&subsection=quiz&id=<?php echo urlencode($_GET['id']); ?>&resultid=<?php echo $result['id']; ?>"><?php echo $nama_lengkap; //esc_html(wp_kses_stripslashes($result['person_name'])); ?></a>
						</strong>
					...
					<td>
						<?php echo $witeldetail->name . " " . $dateldetail->name; ?>
					</td>
					<td><?php if (!empty($result['datetaken'])) { echo date('d-m-y G:i:s',$result['datetaken']); } else { echo '-'; } ?></td>
