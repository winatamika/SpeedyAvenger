<div class="pre-content"></div>
 <div class="timer2" style="float: right; position: fixed; top: inherit; left: 68%;"></div>                <script type="text/javascript">
              /*      jQuery(document).ready(function() {
                        ...
                        var refreshId = setInterval(function() {
                            if (timeSecs != 0) {
                                ...
                                if (timeMins > 0) {
                                    //jQuery(".timer2").html("Waktu tersisa: " + timeMins + " minutes and " + timeSecsRem + " detik");
                                } else {
                                   // jQuery(".timer2").html("Waktu tersisa:" + timeSecsRem + " detik");
                                }
...
<div class="quiz">
<h3><?php echo stripslashes($_SESSION['wpsqt'][$quizName]['sections'][$sectionKey]['name']); ?></h3>
...
?>
<form method="post" action="<?php echo esc_url($url); ?>" onSubmit="if(!confirm('Apakah anda telah yakin dengan soal yang telah di isi?')){return false;}" >
...
<?php if($all == $nomor) : 
	if ($sectionKey == (count($_SESSION['wpsqt'][$quizName]['sections']) - 1)) {
		?><p><input type='submit' id='soaljawab' value='<?php _e('Submit', 'wp-survey-and-quiz-tool'); ?>' class='button-secondary' /></p><?php
	} else {
		?><p><input type='submit' id='soaljawab' value='<?php _e('Next', 'wp-survey-and-quiz-tool'); ?> &raquo;' class='button-secondary' /></p><?php
	}
	echo "<!-- END PAGing -->"; 
	echo "</div>";
endif; ?>
...
/*
if ($sectionKey == (count($_SESSION['wpsqt'][$quizName]['sections']) - 1)) {
	?><p><input type='submit'  value='<?php _e('Submit', 'wp-survey-and-quiz-tool'); ?>' class='button-primary3' /></p><?php
} else {
	?><p><input type='submit' value='<?php _e('Next', 'wp-survey-and-quiz-tool'); ?> &raquo;' class='button-primary3' /></p><?php
}
*/
?>
